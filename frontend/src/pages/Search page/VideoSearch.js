...
const VideoProfileFullSize = ({ openClose, name }) => {
  ...
  //Move video on scroll up and down
  useEffect(() => {
    if (searchedVideos) {
  ...
  //move video for mobile up and down
  useEffect(() => {
    if (searchedVideos) {
      let startY = 0;
      let endY = 0;
  ...
  //Handle like goes after setting states
  useEffect(() => {
    if (like && likedVideo) {
      handleLike();
    }
  ...
  //Check if user liked that video
  useEffect(() => {
